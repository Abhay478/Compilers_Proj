...
%token KW_CLAIM KW_IS KW_GROUP KW_FORGE KW_FN KW_STRUCT KW_ENUM KW_RING KW_FIELD KW_SPACE KW_PRINT KW_LET KW_RETURN KW_IF KW_ELSE KW_WHILE KW_FOR KW_IN KW_SWITCH KW_CASE KW_DEFAULT KW_BREAK KW_CONTINUE KW_CYCLIC KW_BIG_RATIONAL KW_COMPLEX KW_SYMMETRIC KW_ALTERNATING KW_MORPH KW_DIHEDRAL KW_INV_MAT KW_BIGINT KW_MATRIX KW_POLYNOMIAL KW_VEC KW_BUF IDENT PRIMITIVE_DTYPE LIT_INT LIT_FLOAT LIT_STR LIT_CHAR LOGICAL_AND LOGICAL_OR LOGICAL_NOT EQ NEQ GT LT GTEQ LTEQ KW_TRUE KW_FALSE
...
function_header : KW_FN IDENT '(' parameter_list ')' ':' data_type
                | KW_FN IDENT '(' ')' ':' data_type
                ;
...
struct_declaration      : IDENT ':' PRIMITIVE_DTYPE ',' struct_declaration
                        | IDENT ':' KW_BUF '<' PRIMITIVE_DTYPE '>' ',' struct_declaration
                        | IDENT ':' IDENT ',' struct_declaration
                        | IDENT ':' group_data_type ',' struct_declaration
                        | IDENT ':' ring_data_type ',' struct_declaration
                        | IDENT ':' field_data_type ',' struct_declaration
                        | IDENT ':' space_data_type ',' struct_declaration
                        | epsilon
                        ;
*/
/* forges          : KW_LET IDENT ':' PRIMITIVE_DTYPE '=' PRIMITIVE_DTYPE '(' var ')' ';'
                | KW_LET var ':' KW_MATRIX '<' PRIMITIVE_DTYPE '>' '=' 'matrix' '(' matrix_data ')'
                |
*/

forges          : KW_FORGE data_type '(' parameter_list ')' '{' statements '}'

epsilon         : ;

// Morphs          : KW_RING KW_MORPH IDENT '(' IDENT ':'   ')' --
