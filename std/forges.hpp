...
// Cyclic                       - from int
// Symmetric                    - from vector<int>
// Alternating                  - from vector<int>
// Dihedral                     - from int
...
template<int N>
Symmetric<N> forge_Symmetric(int n) {
...
template<int N>
Dihedral<N> forge_Dihedral(int n) {
    return Dihedral<N>(n);
}
...
template<int N>
std::string forge_string(const Dihedral<N>& dih) {
...
static void test() {
    ...
    auto sym = forge_Symmetric<5>(2);
    auto alt = forge_Alternating<5>({1, 2, 3, 4, 0});
    auto dih = forge_Dihedral<5>(3);
